    ...
    for (var o of Object.keys(val0a).sort()) {
      if (o.indexOf(' - ') > 1) {
        ...
        if (!collapsibleSet.has(collapsibleOption)) {
          ...
          availableoptions += '<li id="' + collapsibleSet.size * 1000
            + '" class="list-group-item do-not-drag" style="cursor: pointer; border: none" onclick="grid.toggleCollapseSubOptions(\''
            + collapsibleOption + '\',' + collapsibleSet.size * 1000 + ')">'
            + collapsibleOption + ' ' + gettext('attributes') + collapseIcon + '</li>';
        }
  ...
  showExport: function (only_list, scenario_permissions) {
    ...
    if (only_list)
      content += '<div class="row" id="csvformat">' +
        ...
        gettext("Using this link external applications can pull data from ccAPPS") + '">' +
...
var dashboard = {
  save: function (reload) {
    ...
    $.ajax({
      ...
      type: 'POST',
      contentType: 'application/json; charset=utf-8',
      data: JSON.stringify({ "ccAPPSdb.common.cockpit": results }),
      success: function () {
...
function about_show() {
  ...
  content = '<div class="modal-dialog">' +
    '<div class="modal-content">' +
    '<div class="modal-header">' +
    '<h5 class="modal-title">About ccAPPS</h5>' +
    '<button type="button" class="btn-close" data-bs-dismiss="modal"></button>' +
    '</div>' +
