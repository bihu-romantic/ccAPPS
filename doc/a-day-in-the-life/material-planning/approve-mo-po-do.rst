...
ccAPPS propagates the sales orders and forecast through the supply path.
...
4) | The planner needs to select the records to approve (i.e export to the ERP). Exporting records that don't have
     an execution date pretty close in the future is not a good practice as ccAPPS will continue to replan these records.
5) | The planner needs to click on the "Export to ERP" button to create the records in the ERP.
6) | Next time the interface will be run between the ERP and ccAPPS, these records will be imported with a
     confirmed status in ccAPPS.
