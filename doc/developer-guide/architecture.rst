...
ccAPPS has a 3-tier architecture, typical for most web applications.
...
* | **Browser based front-end**:
  ...
  | ccAPPS supports most modern browsers: Google Chrome, Firefox, Safari,
    Microsoft Edge and Opera.

* | **Web server**:
  | The web server retrieves data from the database and serves the content
    as HTML pages to the user.
  | ccAPPS supports the Apache web server, and also has a built-in
...
A special use case is to use ccAPPS as a **backend planning engine without user interface**.
In this case the planning engine can be embedded in another system. Users
use the other system as the front-end, and ccAPPS's algorithms are used in
