...
custom apps provide to tailor ccAPPS to your business needs.
...
app works within the ccAPPS framework.

In this tutorial we'll be using a `docker container <https://www.docker.com/>`_
to package and deploy your customized version of ccAPPS. While there are other
...
and place its contents under the ccAPPSdb folder.
...
If all went well you can access ccAPPS with your browser now on
http://localhost:9000.
...
.. code-block:: Python
   ...
   ...
   registerAttribute(
       "ccAPPSdb.input.models.Item",  # Class we are extending
       [
           (
...
schema migration allows upgrading between different versions of ccAPPS (or your ap)
...
.. code-block:: none
   ...
   ccAPPSctl makemigrations my_app
...
.. code-block:: Python
   ...
   class MyModelSerializerAPI(ccAPPSListCreateAPIView):
...
be loaded with the command "ccAPPSctl loaddata" or `interactively
...
.. code-block:: Python
   ...
   class Command(BaseCommand):
       # Help text shown when you run "ccAPPSctl help my_command"
...
::
   ...
   # Run the test
   ccAPPSctl test ccAPPSdb.my_app
...
ccAPPS is based on django web application framework. You can dig deeper
by visiting https://www.djangoproject.com, checking out the full documentation
and follow a tutorial.

Another good approach is to study the way the standard apps in ccAPPS
are structured. The full source code of the Community Edition is on
https://github.com/ccAPPS/ccAPPS/tree/master/ccAPPSdb
