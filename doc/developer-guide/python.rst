...
ccAPPS uses the `Python`_ programming language in two different ways:
  ...
  .. code-block:: Python
      ...
      with open('products.csv', newline='') as csvfile:
        ...
        for row in csvreader:
          # The next line creates an item object in the ccAPPS core
          # planning engine.
          ccAPPS.item(name=row[0], description=row[1])

      # Call the planning engine solver to create a fully constrained plan
      ccAPPS.solver_mrp(plantype=1, constraints=15, loglevel=2).solve()
...
executable ccAPPS(.exe), or you can run it from the standard Python
...
- | A very nice example is the code where the planning engine reads
    from the PostgreSQL database: https://github.com/ccAPPS/ccAPPS/blob/master/ccAPPSdb/input/commands/load.py
...
- | Another example is the code to write the plan results from the planning
    engine's memory back to the database: https://github.com/ccAPPS/ccAPPS/blob/master/ccAPPSdb/output/commands.py
