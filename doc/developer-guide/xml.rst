...
Here's a simple annotated example of a data file in ccAPPS's format:
...
The XML format is defined in `XML schema`_ format in https://raw.githubusercontent.com/ccAPPS/ccAPPS/master/bin/ccAPPS.xsd
(use master to see the development version, or replace it with the version number you're using)
or the file /usr/share/ccAPPS/ccAPPS.xsd in a local install.
...
- | The planning engine has a lot of its unit tests written as XML files.
  | See the .xml files in the folders under https://github.com/ccAPPS/ccAPPS/tree/master/test

- | The connector with the Odoo ERP system is implemented as a web service that collects all
    ERP in ccAPPS's XML format. The ccAPPS planning engine connects to this URL, retrieves
...
.. _`that code`: https://github.com/ccAPPS/odoo/blob/16.0/ccAPPS/controllers/outbound.py
