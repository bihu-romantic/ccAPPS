...
.. important::
   ...
   See `this page <../odoo-from-march-2026/installation-and-configuration.html>`_ for
   Odoo connectors installed after March 2026.

The connector has 2 components:

* | An Odoo addon:
  | All mapping logic between the Odoo and ccAPPS data models is in this
    module. The results are accessible on the URL http://odoo_host/ccAPPS/xml
    from which the planning engine will read data in its native XML data format
    and to which it will post the results.

* | A ccAPPS addon:
  | This module gives ccAPPS the capability to connect to Odoo, read the data
    from it, and publish back the results.
  | It also activates additional menus in the ccAPPS user interface.
...
* **Configuring the connector - Odoo side**
  ...
  * | **Install the Odoo addon**
    | The addon code is found in the github repository https://github.com/ccAPPS/odoo.
      Use the branch from the subfolder matching your Odoo version.
    | The addon is also available from the `Odoo app store <https://apps.odoo.com/apps/modules/16.0/ccAPPS/>`_

  * | **Configure the Odoo server**
    | If your Odoo instance is running in multi-database mode you need to
      add the ccAPPS addon as a server wide module. This is achieved by updating an
      option in the Odoo configuration file odoo.conf: "server_wide_modules = web,ccAPPS"
  ...
  * | **Configure the Odoo addon**
    ...
    * | Webtoken key:
      | A secret random string used to sign web tokens for a single signon between
        the Odoo and ccAPPS web applications. Choose a string that is long enough,
    ...
    * | ccAPPS server:
      | URL of your ccAPPS server.
      | Do not include a slash at the end of the URL.

    * | Respect reservations:
      | When this flag is checked, ccAPPS fully respects the material
        reservations of odoo. ccAPPS only plans with the unreserved materials.
      | When this flag is false, ccAPPS plans with the full material availability
    ...
    * | ccAPPS interface user:
      | The user authorized to import data from Odoo into ccAPPS. If left empty,
        any Odoo user can pull data from Odoo into ccAPPS.
      | The ccAPPS interface user should be the same user declared in the odoo.user
        parameter in ccAPPS.
  ...
  * | **Review time zone setting**
    | The time zone preference of the Odoo user utilized by the connector is important.
      Odoo sends all dates to ccAPPS converted to this timezone, and ccAPPS returns dates
      in this timezone.

  * | You can run a **quick test** of the above by opening a web browser to the URL
      http\://<host>:<port>/ccAPPS/xml?database=<db>&language=<language>&company=<company>.
    ...
    | After providing the login details, an XML document will be displayed with
      the data that ccAPPS will read from Odoo.
  ...
  * | **Odoo tuning**
    | The ccAPPS connector is an atypical addon that may require some adjustments on your
    ...
    * | We strongly recommend using a separate PostgreSQL database cluster for ccAPPS. This is 1)
        because Odoo and ccAPPS require different database tuning, and 2) you want both applications
  ...
  * | **Connector development mode**
    ...
    | To enable this option, you need to uncomment and edit two sections in the ccAPPSxml.py file.

    * https://github.com/ccAPPS/odoo/blob/db24d8b4f882e594b02840f549532e356da4e1dd/ccAPPS/controllers/ccAPPSxml.py#L274
    * https://github.com/ccAPPS/odoo/blob/db24d8b4f882e594b02840f549532e356da4e1dd/ccAPPS/controllers/ccAPPSxml.py#L351
...
* **Configuring the connector - ccAPPS side**

  * | **Edit the ccAPPS configuration file /etc/ccAPPS/djangosettings.py**

    * | Assure that the "ccAPPSdb.odoo" is included in the setting
    ...
    * | If ccAPPS and Odoo are installed on 2 different domains (example: https://myccAPPS.ccAPPS.com
  ...
  * **Configure parameters**
    ...
    * | odoo.allowSharedOwnership:
      | By default records read from Odoo aren't editable in ccAPPS. You loose your
...
* **Configuring access rights**
  ...
  access to all ccAPPS functionality. Others users need to be explicitly granted access.

  * | In Odoo, you allow people to access ccAPPS by granting the "ccAPPS user" access
      right.
    | The access is not granted by default.
    | You'll need to switch to developer mode to edit this access right.

  * | All Odoo users with the "ccAPPS user" permission are automatically synchronised
      with ccAPPS.
    | Of course, you can add additional users in ccAPPS beyond these Odoo users.

  * | These Odoo users are added to the "Odoo users" group in ccAPPS. The members of
      that group get complete permissions in ccAPPS.
    ...
    | The permissions are only synchronized in the default, main scenario in ccAPPS.
