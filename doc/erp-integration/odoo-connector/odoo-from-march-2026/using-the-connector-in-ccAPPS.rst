Using the connector in ccAPPS
------------------------------

.. important::

   This page applies to Odoo connectors installed after March 2026.
   See `this page <../odoo-before-march-2026/using-the-connector-in-ccAPPS.html>`_ for
...
* | **Plan generation options to "import Odoo data" and "send recommendations to odoo"**
  | These options are used to assure the ccAPPS is based on the latest Odoo data, and
...
* | **Task "Import data from Odoo"**
  | The execute screen has an *import data from odoo* command that
    allows to import the Odoo data in ccAPPS database.
...
* | **Task "Pull all demand history from Odoo"**
  | The execute screen has a *pull all demand history from odoo* command that
    ...
    that can be used from a fresh ccAPPS database to pull all the demand history
...
* | **Task "Export data to Odoo"**
  ...
  | The plan exported to Odoo is a subset of the plan which passes
    ...
    only be exported manually from ccAPPS to Odoo: see below.
...
* | **An incremental export from the ccAPPS user interface for
  ...
  | A typical usage is to automatically export the proposed purchase for
    ...
    the proposed plan in ccAPPS for expensive or slow moving items.
