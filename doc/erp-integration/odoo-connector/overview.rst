...
`what Odoo cannot do for planners <https://ccAPPS.com/blog/five_things_odoo_mrp_doesnt_do/>`_
...
* | Live data integration.
  | The connector reads the data directly from Odoo and writes master and
    transactional data in ccAPPS database. The planner can then run a plan in ccAPPS
    and either export all the plan in Odoo or selects which data to export.

* | User interface integration.
  | Users can access ccAPPS screens from the Odoo interface, without
...
Below are the slides presented during the Odoo user conference in October 2022 where we describe how ccAPPS
