...
ccAPPS provides an integration with `Openbravo <http://www.openbravo.com>`_, a
...
* Two-way integration:
  ...
  * Synchronizes the ccAPPS database with items, locations, bill of materials,
  ...
  * Uploads new production requirements, purchase requisitions and expected
    delivery date of sales orders from ccAPPS to Openbravo/Etendo.
...
* You can still maintain additional data in the ccAPPS user interface. I.e.
  Openbravo/Etendo doesn't need to be the only source of data for your ccAPPS model.
...
Importing data from Openbravo or Etendo into ccAPPS
----------------------------------------------------

You can run the import interface in 3 ways:

* | **Interactively from the ccAPPS user interface.**
...
* | **From the command line.**
  | The ccAPPSctl command is useful when you want to run the interface
  ...
  ::
    ...
    ccAPPSctl openbravo_import
    ccAPPSctl openbravo_import --delta=7

* | **Through a web API.**
  | The web API is the proper method to automate the integration on the ccAPPS
    cloud servers.
...
It is possible to combine both the incremental and bulk export in the same ccAPPS
...
above the threshold are then reviewed in ccAPPS by the planner, and export
incrementally upon the planners' approval.

Installation and configuration
------------------------------

Most of the configuration is happening on ccAPPS side.
...
* **Configuring the connector - Openbravo/Etendo side**
  ...
  * | The connector uses the `standard XML REST web services <http://wiki.openbravo.com/wiki/XML_REST_Web_Services>`_
      ...
      integrate into ccAPPS.

* **Configuring the connector - ccAPPS side**

  * | **Edit the configuration file djangosettings.py**
    | The file is found under /etc/ccAPPS.
    | Assure that the ccAPPSdb.openbravo is included in the setting
      INSTALLED_APPS which defines the enabled extensions. By default
      it is not enabled.

  * | **Migrate your ccAPPS database**
  ...
  * | **Configure the following parameters**
    | In the ccAPPS user interface, the menu item 'admin/parameters' opens a
...
ccAPPS. The connector is quite likely to require some customization to fit
the particular setup of the Openbravo/Etendo and the planning requirements in ccAPPS.
