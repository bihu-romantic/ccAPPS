...
ccAPPS by using the concept of skills.
...
`Check this feature on a live example <https://demo.ccAPPS.com/resource-alternate/data/input/resource/>`_
...
* We are modelling an operation that assembles round tables. To assemble a round table we 
  ...
  `supply path of a sales order <https://demo.ccAPPS.com/resource-alternate/supplypath/demand/Demand%2001/>`_.
...
* | The machine parks consists of "machine A", "machine B" and "machine C". Only machine A and machine B
  ...
  | Check out the
    `resource <https://demo.ccAPPS.com/resource-alternate/data/input/resource/>`_,
    `skill <https://demo.ccAPPS.com/resource-alternate/data/input/skill/>`_
    and `resource skill <https://demo.ccAPPS.com/resource-alternate/data/input/resourceskill/>`_
...
* | The `operation resources table <https://demo.ccAPPS.com/resource-alternate/data/input/operationresource/>`_
  ...
  * **Priority**
    ...
    When search mode is set to *priority*, ccAPPS will pick the resources by priority 
    (lowest values first, defined in resource skill table) as long as the demand is planned on time.
    
  * **Minimum cost search mode**
    
    When search mode is set to *minimum cost*, ccAPPS will pick the resource that minimizes the cost of production.
  ...
  * **Minimum penalty**
    ...
    When search mode is set to *minimum penalty*, ccAPPS will pick the resource that minimizes the penalty.
    ...
    ccAPPS will then pick the second resource to produce the second demand (as the first resource will pay a
    penalty because it has been picked for the first demand and can only produce earlier). Then, for the third
    demand, both resources will have the same penalty and ccAPPS will pick again the first one.
    
  * **Minimum cost plus penalty**
    
    Obviously, this option is a combination of the cost and the penalty. ccAPPS will compute both the cost and
    the penalty for a resource and will pick the one that minimizes the sum.
  
  For the machine park, we have set the search mode to *priority* as we want ccAPPS
...
* | In the generated plan we can review that, as long as the demand can be met on time (or early), 
  ...
  | Try adjusting the due date or quantity of the 
    `sales orders <https://demo.ccAPPS.com/resource-alternate/data/input/demand/>`_
     to make some demands late. You will see the alternates *machine B* and *Antonio* start
     to be used.

  | The `resource detail report <https://demo.ccAPPS.com/resource-alternate/data/input/operationplanresource/>`_
    and `plan editor <https://demo.ccAPPS.com/resource-alternate/planningboard/>`_
