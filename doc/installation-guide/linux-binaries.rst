...
#. **Download the installation package**
   ...
   The Community Edition is a free downloaded from github at https://github.com/ccAPPS/ccAPPS/releases.

   The Enterprise Edition is available to customers from github at https://github.com/ccAPPS/ccAPPS-enterprise/releases

#. **Install and tune the PostgreSQL database**

   Install postgreSQL 17, 16, 15 (in this order of preference).

   ccAPPS assumes that the database uses UTF-8 encoding.

   ccAPPS needs the following settings for its database connections. If these
   ...
   ccAPPS can communicate with the PostgreSQL server using either a) Unix
   domain sockets ('local' in pg_hba.conf) or b) TCP IP4 or IP6 sockets.

   ccAPPS can authenticate on the PostgreSQL database using either a) a
...
#. **Configure ccAPPS**
   ...
   #. **/etc/ccAPPS/djangosettings.py - DATABASES**
      ...
      ::
        ...
        DATABASES = {
          'default': {
            ...
            'NAME': 'ccAPPS',
            'USER': 'ccAPPS',     # Role name when using md5 authentication.
                                   # Leave as an empty string when using peer or
                                   # ident authencation.
            'PASSWORD': 'ccAPPS', # Role password when using md5 authentication.
                                   # Leave as an empty string when using peer or
                                   # ident authencation.
            ...
            'FILEUPLOADFOLDER": os.path.normpath(
                os.path.join(ccAPPS_LOGDIR, 'data', 'default')
            ),
            'SQL_ROLE': 'report_role',
            'ccAPPS_PORT': '127.0.0.1:8002',   # Enterprise Edition only
            'SECRET_WEBTOKEN_KEY': SECRET_KEY,
            'TEST': {
              'NAME': 'test_ccAPPS' # Database name used when running the test suite.
              }
            },
         ...
      ...
      you also need to assign a unique port number in the ccAPPS_PORT setting.
   ...
   #. **/etc/ccAPPS/djangosettings.py - SECRET_KEY**
   ...
   #. **/etc/ccAPPS/djangosettings.py - DATE_STYLE**
   ...
   #. **/etc/ccAPPS/djangosettings.py - INSTALLED_APPS**
      ...
      ::
        ...
        INSTALLED_APPS = (
          ...
          'ccAPPSdb.wizard',              << COMMENT IF MODEL BUILDING WIZARD ISN'T NEEDED
          'ccAPPSdb.input',
          #'ccAPPSdb.odoo',             # << UNCOMMENT TO ACTIVATE THE ODOO INTEGRATION
          #'ccAPPSdb.erpconnection',    # << UNCOMMENT TO ACTIVATE THE GENERIC ERP INTEGRATION
          'ccAPPSdb.metrics',
          'ccAPPSdb.output',
          'ccAPPSdb.execute',
          'ccAPPSdb.common',
          ...
          'ccAPPSdb.reportmanager',
          'ccAPPSdb.executesql',
          )

   #. **/etc/ccAPPS/djangosettings.py - TIMEZONE**

      | Edit the "TIME_ZONE" variable if required. By default, the server time zone
        (where ccAPPS is installed) will be used for both the database and the server.
   ...
   #. | **/etc/httpd/conf.d/z_ccAPPS.conf**
...
#. **Create the database schema**
   ...
   Note that the ccAPPSctl command is only accessible to members of the "ccAPPS"
...
and configuring ccAPPS with a PostgreSQL database on an Ubuntu server.
...
::
  ...
  sudo su - postgres
  psql template1 -c "create user ccAPPS with password 'ccAPPS' createrole"
  psql template1 -c "create database ccAPPS encoding 'utf-8' owner ccAPPS"
  psql template1 -c "create database scenario1 encoding 'utf-8' owner ccAPPS"
  psql template1 -c "create database scenario2 encoding 'utf-8' owner ccAPPS"
  psql template1 -c "create database scenario3 encoding 'utf-8' owner ccAPPS"
  exit
  # The default ccAPPS configuration uses md5 authentication on unix domain
  ...
  # Install the ccAPPS binary .deb package and the necessary dependencies.
  sudo apt -f ./*.deb

  # Create ccAPPS database schema
  sudo ccAPPSctl migrate --noinput
