...
ccAPPS allows to set a quota to limit the usage of the disk space.
...
- | The disk space of the ccAPPS log folder. The ccAPPS log folder contains:
  ...
  | - The database dump files generated by the `contact ccAPPS support <../command-reference.html#backup>`_ command.
...
located in the */etc/ccAPPS* folder.

For an on premise installation of ccAPPS, the MAXSTORAGE variable is not set by default and the disk space usage is unlimited.

Free trial users have a quota of 1GB during their 30-day evaluation period.

The disk space usage and the allocated quota is visible in the *Help/About ccAPPS* menu.
...
- Delete any dump file generated by the `contact ccAPPS support <../command-reference.html#backup>`_ command.
