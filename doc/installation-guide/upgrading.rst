...
ccAPPS allows migrating an existing installation to a new release without any loss of data.
...
#. **Backup your old environment**
   ...
   databases and b) the configuration file /etc/ccAPPS/djangosettings.py.

#. **Upgrade the PostgreSQL database**

   ccAPPS requires 13 or higher. If you're on an older version, upgrading
   your PostgreSQL database is the first step.

   When you move your ccAPPS databases to a new server, there are some special
   ...
   #. ccAPPS uses 2 database roles to access the database. These are configured as USER
      (for full access) and SQL_ROLE (for read-only access to some tables in report manager)
      in the DATABASES section of your /etc/ccAPPS/djangosettings.py file.
...
#. **Install the new ccAPPS release.**
...
#. **Update the configuration file /etc/ccAPPS/djangosettings.py**
...
::
  ...
  sudo dpkg --force-confold -i <ccAPPS installation package>.deb

  # Manually edit the /etc/ccAPPS/djangosettings.py file. The previous line
