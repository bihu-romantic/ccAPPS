...
You can already check out a `preview <https://demo-preview.ccAPPS.com>`_.
...
- | Extra robustness to deal with negative safety stocks.
  | Some people throw ugly data to ccAPPS. We treat negatives safety stocks values now
    as a zero.

.. rubric:: Odoo integration

- | 18, 19: The default behavior for bringing draft purhase orders (aka RFQs) into
    ccAPPS has been adjusted.
...
- | 17, 18, 19: Bug fix for the bulk export from ccAPPS to odoo in multi-company
    setups.
...
- | 17, 18, 19: For security reasons, a new field called "ccAPPS interface user" was added
    to the ccAPPS settings in Odoo. Only that user can run an "import from Odoo" task
    in ccAPPS.
...
- | The number of problems generated by the planning algorithm has been
    drastically reduced.
  | Experience learns that these alerts aren't very efficient in helping
    planners to review or resolve issues in the plan.
  | ccAPPS now no longer generates problems for: unplanned demands,
    demands planned late, demands planned early, operations before current,
...
- | When exporting manufacturing orders from ccAPPS to Odoo, the connectors are
    now sending the consumed and produced items. This can be useful in situations
    where alternate materials exist.

- | User accessing ccAPPS from the Odoo user interface could end up in
...
- | The emails sent out by ccAPPS now look much prettier.
...
- | 17, 18: Updated logic for synchronizing Odoo users to ccAPPS in multi-company setups.
  | Earlier releases activated ccAPPS access only in the default company of an Odoo user.
  | Now we activate the ccAPPS access in all allowed companies of an Odoo user.

- | 17, 18: Bug fix for the due date quoting module in multi-company setups.
  | The quote message was sent to the ccAPPS instance of the user's default company.
    It should be sent to the ccAPPS instance of the sales order's company instead.
...
- | Planners can now add `extra scenarios <user-interface/what-if-scenarios.php#scenario-management>`_
  ...
  | The maximum number of scenarios depends on the ccAPPS version (community, enterprise, cloud, cloud plus).
...
- | The docker container is now running as the user "ccAPPS" rather than as root for extra security.
...
- | The automatically keep the disk space for log files within acceptable boundaries, ccAPPS
...
- | All Odoo versions. When exporting POs, MOs or DOs from the sales order table of ccAPPS to Odoo,
...
- | 17, 18: New field "post operation time" on Odoo BOM operations.
  | This field maps into the post-operation time field in ccAPPS, which defines a time
    gap ccAPPS tries to leave between the operations.
...
- | In version 8.3 of ccAPPS, we introduced the possibility to filter MOs/POs/DOs using
...
- | 17, 18: Bug fix. The stock reservations by sales orders were not correctly
    considered. ccAPPS incorrectly planned for the full quantity rather than only the
...
- | 17, 18: The customer names are replaced with their Odoo IDs in ccAPPS.
...
- | Performance optimization for configurations with a high number of scenarios.
  | When many scenarios are active we noticed the user interface performance
    is impacted, and ccAPPS also needed an increasing number of database connections.
...
- | 18: Keep the language selection of a user identical in Odoo and ccAPPS.
  | Displaying both user interfaces in a different language isn't very nice, isn't it?

- | 18: Synchronize products tags from Odoo into the item category in ccAPPS.
...
- | 17, 18: Ability to export a remark from ccAPPS into Odoo on
    manufacturing orders, distribution orders or purchase orders.
  | The remark shows up in the "source" field in odoo.

- | 17, 18: Blanket order is now populated in Odoo when exporting purchase orders from ccAPPS.

- | 17, 18: Various bug fixes to support configurations where Odoo schedules work orders,
    while ccAPPS only plans at the manufacturing order level.
...
- | Updated rules for default permissions of new users.
  ...
  | The new behavior makes the first steps in ccAPPS after an initial intallation
...
- | The development container have been improved and this is now the easy, simple
    and recommended method to do any development on ccAPPS or ccAPPS addons.
  | Within 5 minutes any developer can now have a complete development
    environment ready.

.. rubric:: Odoo integration

- | 17: To keep the names shorter in ccAPPS we now a) use the warehouse short code
...
- | Bug fix: Manufacturing orders for make-to-order products couldn't be updated
    with new dates from ccAPPS.

.. rubric:: System performance

- | Various performance improvements for ccAPPS server under heavy load.
...
- | 17: Performance improvements of about 20% to 30% when importing Odoo data into ccAPPS.
...
- | Introduction of machine learning forecasting capabilities to ccAPPS with the addition of a
    new app based on the `orbit library <https://orbit-ml.readthedocs.io/en/latest/>`_.
  | The machine learning forecast app is still in beta-testing and would require the ccAPPS team to
...
- | 15, 16, 17: Addition of the expiry dates management. Lot expiry dates and product expiration time
    are retrieved by the connectors in ccAPPS. This functionality requires the
    `shelf life <apps/shelflife.html>`_ app to be installed on the ccAPPS side.
    Note that the shelf life app is only available in the enterprise version of ccAPPS.

- | 15, 16, 17: The product category in Odoo is now used to build the item hierarchy in ccAPPS.
...
- | 15, 16, 17: Addition of a new command *Pull demand history from Odoo*. This command uses the
    XML RPC interface of Odoo and pulls all the sales history in the ccAPPS database.

.. rubric:: System administration

- | A lot of settings in the /etc/ccAPPS/djangosettings.py configuration file can now
...
- | Ubuntu 24 LTS is now the supported operating system.
  | For all other operating systems a docker container is the way to run ccAPPS.
...
- | You can now configure ccAPPS to hide the hours, minutes and seconds from
...
- | 16, 17: Support for Odoo make-to-order products.
  | Odoo automatically creates the manufacturing orders and purchase orders for
    such products. The ccAPPS connector respects their links to the source sales order
...
- | 17: Addition of `quoting <erp-integration/odoo-connector/using-the-connector-in-odoo.html#quoting-capabilities>`_ capabilities
  ...
  | A new *ccAPPS Quotes* screen is added to get a promised date for a product.
...
- | Ability to customize the "export plan results" task.
  | Until now you had to rely on the standard export logic or write a ccAPPS app to tailor the
...
- | Switching work orders to alternate work centers in a pool is now fully supported.
  | Existing assigment were already sent from Odoo to ccAPPS. When reassiging
    in ccAPPS the new assignment is communicated back to odoo.

- | 15, 16, 17: The mapping of calendars now includes the Odoo identifier.
  | This is needed to guarantuee uniquess of the calendars.

- | 15, 16, 17: The ccAPPS item name is now mapped to the Odoo product internal reference,
    unless the internal reference is not unique in Odoo.

- | 15, 16, 17: Allow export of approved and confirmed purchase orders from ccAPPS to Odoo to
    update various fields of the Odoo purchase order lines (quantity, receipt date, item...).

- | 15, 16, 17: Creation of purchase orders in Odoo (upon export from ccAPPS) uses generic
...
- | 16, 17: Mapping of Odoo "replenish on order" products into ccAPPS
    make-to-order items.
  | For such items both Odoo and ccAPPS maintain a hard link between
...
- | Part of our cloud servers are now running in the Amazon data center in
  ...
  | Cloud customers whose ccAPPS infrastructure was moved will be notified.

.. rubric:: Legal

- | Refreshed the `privacy policy <https://ccAPPS.com/privacy-policy-2/>`_.
...
- | ccAPPS is now installable as an app on your tablet and smartphone.
...
- | A new `apps screen <user-interface/getting-around/apps.html>`_ allows superusers
  ...
  | Until now, administrators had to edit the INSTALLED_APPS setting in the
    /etc/ccAPPS/djangosettings.py configuration file.
...
- | The 'about ccAPPS' option in the help menu is now gone.
...
- | The approval button to export a proposed purchase order, distribution order
    or manufacturing order from ccAPPS to Odoo is now a button rather than a
...
- | The command to `back up the database <command-reference.html#backup>`_ is renamed
    to `contact ccAPPS support <command-reference.html#backup>`_ as this is the official way
    for sharing a database dump with the ccAPPS support.
...
- | The license of the Community Edition changes from `AGPL <https://wikipedia.org/wiki/Affero_General_Public_License>`_ to
  ...
  | Read `this post <https://ccAPPS.com/blog/why-we-are-changing-our-license-from-agpl-to-mit/>`__ to learn more.
...
- | The forecasting module is moving into the Community Edition.
  ...
  | Read `this post <https://ccAPPS.com/blog/the-forecast-module-goes-open-source/>`__ to learn more.

Join the `webinar on Thursday June 8 at 4 PM Central European Time <https://ccAPPS.com/webinar_ccAPPS_8.ics>`_ to hear the full story
...
- | 15 & 16: The progress of work orders is now synchronised between Odoo and ccAPPS.
  ...
  | The connector now creates ccAPPS operations specific to each manufacturing order to
    ...
    Only with a dedicated operation can we assure that ccAPPS correctly represents the
    Odoo data.
...
- | 15 & 16: On manufacturing orders the connector now picks up the quantity actually
  ...
  | When pieces are scrapped or lost in any other way in the factory, the manufacturing
    orders in ccAPPS will now adjust accordingly.
...
- | Support for tool resources.
  ...
  | The tools are modelled as resources in ccAPPS, with the subcategory field set to true.
...
- | Improved level-loading logic for assigning resources to approved and
  ...
  | When approved and confirmed manufacturing orders are loaded from your
    ERP without any assigned resources, ccAPPS automatically assign resources.
...
- | The ccAPPS connector is now available from the Odoo app store.
  | You can downloaded and install from https://apps.odoo.com/apps/modules/16.0/ccAPPS/
...
- | 14 & 15 & 16: When exporting RFQ purchase orders we now populate the order deadline date
    and receipt date.
  | We put the earliest order start and end date of the exported ccAPPS records in these
...
- | Addition of a *debug report* link in the *Help/About ccAPPS* window.
...
- | Support for Ubuntu 18 is dropped.
  | The ccAPPS team will only support Ubuntu 20. Use a Docker container
...
- | 14 & 15 & 16: When planners manually approve purchase and manufacturing orders, the
  ...
  | In previous releases, the generic account running the ccAPPS connector was used
...
- | 14 & 15 & 16: A new configuration flag "respect_reservations" is added for the connector.
  | When this flag is checked, ccAPPS fully respects the material reservations
    of odoo. ccAPPS only plans with the unreserved materials.
...
- | The installation process has been simplified. Except for the postgresql configuration,
    ...
    - The installer now automatically can migrate your databases during
      a ccAPPS upgrade. A prompt is shown to confirm whether or not you want to do this.
...
- | v14 & 15: Products of type "consumable" are no longer interfaced to ccAPPS.
...
- | v14: Bug fix for mapping Odoo reorderpoints to ccAPPS.
...
- | A django patch has been backported to the ccAPPS django fork.
...
- | v14: Bug fix, subcontractor lead time was interpreted by ccAPPS in
...
- | V14: Extra links from the Odoo menus to the ccAPPS screens.
...
- | V14: New configuration to send stack traces from the connector back to your ccAPPS
...
- | V14: Ability to limit the data to a single Odoo company only.
  | By default, the connector extracts data for all allowed companies the connector user
    has access to into a single ccAPPS dataset.
  | With the new option you can separate the ccAPPS datasets per Odoo company.
...
- | The django version is bumped up from 2.2 LTS to 3.2 LTS.
  ...
  |  sudo -H pip3 install --force-reinstall -r https://raw.githubusercontent.com/ccAPPS/ccAPPS/6.21.0/requirements.txt
...
- | v14: The connector is now consistently using the timezone preference of the odoo
  ...
  | In previous releases you could see some unexpected time shifts when the odoo
    and ccAPPS servers run in different timezones.
...
- | It is now possible to approve a workorder in ccAPPS to approve the complete
    manufacturing order to odoo.
  | In previous releases users had to filter out the routing manufacturing orders
    in ccAPPS and only approve those. Quite tedious, isn't it?
...
- | V14: Support for the "consumed in operation" field for bill of material components.
  | In previous releases ccAPPS consumed all bill of material components in the
    first routing step. With the extra mapping we can now consume some components
    at other steps.

- | V14: Changed naming convention for operations in ccAPPS. With the Odoo id at the end, the
    new name "item @ location id" is more readable than "id item @ location".

- | V14: Changed naming convention for customers in ccAPPS. With the Odoo id at the end, the
...
- | V14: Handling of multi-timezone situations when importing and exporting manufacturing orders,
  ...
  | When the ccAPPS and Odoo servers reside in different timezone things get mixed up in
    previous releases.
...
- | The security HTTP header now includes the newer
    ...
    file.
  | Unless you're embedding ccAPPS web pages as an iframe in your application
...
- | The `old ccAPPS-user group <https://groups.google.com/g/ccAPPS-users>`_ has
    been closed.
  | `Github Dicussions <https://github.com/ccAPPS/ccAPPS/discussions>`_ are the
...
- | The Continuous Integration (CI) software building process is now fully transparent
    run on `Github Actions <https://github.com/ccAPPS/ccAPPS/actions>`_.
...
- | The Windows installer for the Community Edition has been removed. ccAPPS is an
...
- | The docker container is now fully production ready.
  ...
  | The images for the Community Edition can be pulled from the
    `Github Container Registry <https://github.com/orgs/ccAPPS/packages/container/package/ccAPPS-community>`_.
...
- | Bug fix: A solver issue with unconstrained resource has been corrected. See
    https://github.com/ccAPPS/ccAPPS/issues/381
...
- | With a new field "quantity_completed" on manufacturing orders, ccAPPS now can
...
- | ccAPPS's user interface has quite some nice capabilities that are generic and reusable
    in other domains.
  | We have copied these out into a separate project https://github.com/ccAPPS/ccAPPS-data-admin.
...
- | Self-diagnoses check for required python third party packages.
  | Missing python package will now be reported as an error whenever you run ccAPPSctl.
...
- | ccAPPS now uses machine-assisted translations.
...
- | ccAPPS is now using the cmake build system rather than the archaic autotools.
...
- | Addition of the `data source URL <user-interface/getting-around/exporting-data.html>`__ in the export dialog
    for easier export of ccAPPS data into Excel. External applications can now directly pull ccAPPS
...
- | Authentication to all URLs of the application is now possible with
    ...
    This feature makes it easy for other applications to pull data or embed ccAPPS.
...
- | New demand history, purchase order history and inventory history widgets on the
  ...
  | ccAPPS will now record historical plan data. In following releases you can expect
...
- The Odoo addon is moved to its own github repository: https://github.com/ccAPPS/odoo
...
- | A vulnerability in the django web application framework was identified and corrected.
  ...
  | By default ccAPPS doesn't configure an SMTP mail server. The password reset functionality
...
- | The third party components we depend on have been upgraded to new releases. Most
  ...
  | When upgrading a linux installation from a previous release, use the following command
    to upgrade the Python packages. On Windows the new packages are part of the installer.
      sudo -H pip install --force-reinstall -r https://raw.githubusercontent.com/ccAPPS/ccAPPS/6.0.0/requirements.txt
...
- | The modelling wizard that guides new users in loading their first data in ccAPPS is completely
...
- A new guided tour is available. Previous guided tour was a journey around the different pages
  and features of ccAPPS. New guided tour is composed of use case questions, illustrated in
  a short video.
...
- | The login form now offers the option to remember me the login credentials. This avoids that
    a user has to login every time a browser session on ccAPPS is started.
...
- Command ccAPPS_run is renamed to runplan.
- Command ccAPPS_runserver is renamed to runwebserver.
- Command ccAPPS_copy is renamed to scenario_copy.
- Command ccAPPS_importfromfolder is renamed to importfromfolder.
- Command ccAPPS_exporttofolder is renamed to exportfromfolder.
- Command ccAPPS_flush is renamed to empty.
- Command ccAPPS_backup is renamed to backup.
- Command ccAPPS_restore is renamed to restore.
- Command ccAPPS_simulation is renamed to simulation.
- Command ccAPPS_createbuckets is renamed to createbuckets.
- Command ccAPPS_createmodel is renamed to createmodel.
- Command ccAPPS_loadxml is renamed to loadxml.
- Command ccAPPS_runworker is renamed to runworker.
