...
* The date format exported and imported by ccAPPS is 'YYYY-MM-DD HH:MM:SS'.
  Microsoft Excel tends to export dates in your local format, which can cause
  problems when you save the file again and try to importing it back in ccAPPS.
...
The video below demonstrates how to pull ccAPPS data in an Excel spreadsheet.
...
* Click on the "Copy to clipboard" button so that ccAPPS copies the data source URL into your clipboard. Note that this functionality
...
* Your credentials need to be populated in Excel so that it can connect to your ccAPPS instance.
  Choose the "basic" option and enter your ccAPPS username and password. Note that, as long as the connection uses the https protocol
  (which is always the case for the ccAPPS cloud users), the crendentials are encrypted.

.. image:: ../_images/excel-data-source-url-3.png
   :alt: Exporting data

* If Excel has been able to connect to your ccAPPS instance, Excel should display data from your report in a dedicated window:
...
* At this point, you have the choice to either load your data or most likely transform your data to select which columns to display,
  ...
  your data using the data source URL will differ from the columns selection you have on ccAPPS. Once transformed, you can then load your data.

* The magic happens when the report you have exported using the data source URL is modified in ccAPPS, you have the possibility in Excel to
  refresh your data in a simple click using the "refresh" option. This will automatically create a connection to your ccAPPS instance and update
