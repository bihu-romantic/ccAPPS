...
ccAPPS automatically detects which of the two data formats you're using.
...
the format of the export file is such that it can be reread into ccAPPS.
...
If it's present ccAPPS skips all data rows above the row where the autofilter
data table starts.
...
.. Caution::
   If you're not very familiar with the relation between the objects in ccAPPS's
...
* The date format expected by ccAPPS is 'YYYY-MM-DD HH\:MM\:SS'.
